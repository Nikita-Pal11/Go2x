...
type AvatarStage = 'seed' | 'sprout' | 'sapling' | 'flower' | 'tree' | 'twoTrees' | 'forest'
...
export function EvolvingAvatar() {
  ...
  const containerBg = useMemo(() => {
    switch (stage) {
      case 'seed': return 'bg-amber-900/20' // Soil
      case 'sprout':
      case 'sapling': return 'bg-emerald-900/10' // Grass
      case 'flower':
      case 'tree': return 'bg-sky-500/10' // Sky
      case 'twoTrees':
      case 'forest': return 'bg-green-950/20' // Deep Forest
    }
  }, [stage])
  ...
  return (
    <div className="flex flex-col items-center gap-6 relative">
      <motion.div 
        layout
        className={`w-48 h-48 rounded-[3rem] ${containerBg} border border-white/5 flex items-center justify-center relative overflow-hidden glass shadow-2xl transition-colors duration-1000`}
      >
        {/* Background elements */}
        <AnimatePresence>
...
<<<<<<< HEAD
              className="absolute font-black text-primary pointer-events-none drop-shadow-glow"
=======
...
<<<<<<< HEAD
        <motion.p 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="text-xs font-black text-primary uppercase tracking-[0.2em]"
        >
          Level {Math.floor(xp / 100) + 1} — {stage}
        </motion.p>
        <p className="text-sm text-white/60 font-medium">
          {avatarState === 'sick' ? 'Needs attention! Maintain your streak.' : 'Growth in progress...'}
        </p>
      </div>
    </div>
  )
...
function AvatarStageRenderer({ stage, state }: { stage: AvatarStage, state: AvatarState }) {
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        {/* Ground */}
<<<<<<< HEAD
        <ellipse cx="60" cy="110" rx="40" ry="10" fill="rgba(255,255,255,0.05)" />

        {stage === 'seed' && (
          <motion.circle 
            cx="60" cy="100" r="8" fill="#5c4033" 
...
<<<<<<< HEAD
        {stage === 'sapling' && (
          <g transform={isSick ? "rotate(15 60 110)" : ""}>
            <rect x="57" y="70" width="6" height="40" rx="2" fill={trunkColor} />
            <motion.path d="M60 75 Q40 65 35 75 Q40 85 60 75" fill={leafColor} animate={{ rotate: [-2, 2, -2] }} transition={{ repeat: Infinity, duration: 4 }} />
            <motion.path d="M60 85 Q80 75 85 85 Q80 95 60 85" fill={leafColor} animate={{ rotate: [2, -2, 2] }} transition={{ repeat: Infinity, duration: 4.5 }} />
          </g>
        )}

        {stage === 'flower' && (
          <g transform={isSick ? "rotate(20 60 110)" : ""}>
            <rect x="57" y="60" width="6" height="50" rx="3" fill={trunkColor} />
            {/* Leaves */}
            <path d="M60 90 Q40 80 35 90" stroke={leafColor} strokeWidth="6" strokeLinecap="round" />
            <path d="M60 100 Q80 90 85 100" stroke={leafColor} strokeWidth="6" strokeLinecap="round" />
            {/* Flower */}
            <motion.g 
              animate={{ scale: isSick ? 0.9 : [1, 1.05, 1], rotate: isSick ? 20 : [0, 5, -5, 0] }}
              transition={{ repeat: Infinity, duration: 3 }}
            >
              <circle cx="60" cy="55" r="12" fill={flowerColor} filter="blur(1px)" />
=======
        {stage === "sapling" && (
          <g transform={isSick ? "rotate(15 60 110)" : ""}>
            <rect
...
<<<<<<< HEAD
        {stage === 'tree' && (
          <g transform={isSick ? "rotate(10 60 110)" : ""}>
            <rect x="54" y="50" width="12" height="60" rx="4" fill={trunkColor} />
            <motion.circle 
              cx="60" cy="45" r="30" fill={leafColor} className="opacity-80"
              animate={{ scale: [1, 1.02, 1] }} transition={{ repeat: Infinity, duration: 5 }}
            />
            <motion.circle 
              cx="45" cy="50" r="20" fill={leafColor} className="opacity-90"
              animate={{ scale: [1, 1.04, 1] }} transition={{ repeat: Infinity, duration: 6, delay: 0.5 }}
            />
            <motion.circle 
              cx="75" cy="50" r="20" fill={leafColor} className="opacity-90"
              animate={{ scale: [1, 1.03, 1] }} transition={{ repeat: Infinity, duration: 5.5, delay: 1 }}
=======
        {stage === "tree" && (
          <g transform={isSick ? "rotate(10 60 110)" : ""}>
            <rect
