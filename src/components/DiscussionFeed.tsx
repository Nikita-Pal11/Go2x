...
import { MessageSquare, ExternalLink, Hash, ThumbsUp, MessageCircle, AlertCircle, Sparkles } from 'lucide-react'
...
<<<<<<< HEAD
            if (!topic) return acc
            return acc + (post.title.toLowerCase().includes(topic.toLowerCase()) ? 1 : 0)
          }, 0)
        }))

        const filteredPosts = scoredPosts
          .filter((post: any) => (post.ai_score || 0) >= 0) // Keep all for now to show something, but rank by score
          .sort((a: any, b: any) => (b.ai_score || 0) - (a.ai_score || 0) || b.score - a.score)
          .slice(0, 6)
...
<<<<<<< HEAD
        ...
        >
          <div className="p-3 rounded-2xl bg-primary/10 border border-primary/20 glow">
            <MessageSquare className="w-8 h-8 text-primary" />
          </div>
          Knowledge Exchange
        </motion.h3>
        <p className="text-muted-foreground font-medium pl-14">
          Real-time insights synthesized from <span className="text-primary font-bold">#{currentTopic}</span> discussions
=======
        <motion.h3
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
...
<<<<<<< HEAD
        <div className="absolute -top-24 -right-24 w-64 h-64 bg-primary/10 rounded-full blur-[100px] pointer-events-none group-hover:bg-primary/20 transition-colors duration-700" />
        <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-purple-500/10 rounded-full blur-[100px] pointer-events-none group-hover:bg-purple-500/20 transition-colors duration-700" />

        <div className="glass rounded-[3rem] p-10 relative overflow-hidden border-white/10">
=======
...
<<<<<<< HEAD
            ...
            <span className="text-[10px] font-black text-primary uppercase tracking-[0.2em] px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 backdrop-blur-md">
              Neural Link Active
=======
                  animate={{
                    scaleY: [1, 1.5, 1],
...
>>>>>>> axdeep-ui-new
                    ...
                    exit={{ opacity: 0 }}
<<<<<<< HEAD
                    className="p-8 rounded-[2rem] bg-white/5 border border-white/5 space-y-6 animate-pulse"
                  >
...
<<<<<<< HEAD
                    <p className="text-xl font-bold text-white">Interface Disruption</p>
                    <p className="text-muted-foreground max-w-xs mx-auto">The AI is currently recalibrating its sub-space communications. Please standby.</p>
=======
...
<<<<<<< HEAD
                    ...
                    className="p-8 rounded-[2rem] bg-white/[0.03] border border-white/5 hover:border-primary/40 hover:bg-white/[0.06] transition-all duration-300 group/card flex flex-col justify-between shadow-2xl overflow-hidden relative"
                  >
                    ...
                    <div className="space-y-5 relative z-10">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black text-primary uppercase tracking-tighter px-3 py-1 rounded-lg bg-primary/10 border border-primary/20">
                          Curated Insight
                        </span>
                        <div className="flex items-center gap-4 text-muted-foreground text-[10px] font-bold">
                          <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/5">
                            <ThumbsUp className="w-3 h-3 text-orange-500" /> {post.score}
                          </div>
                          <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/5">
                            <MessageCircle className="w-3 h-3 text-blue-500" /> {post.answer_count}
                          </div>
                        </div>
                      </div>
                      
                      <h4 
                        className="text-lg font-bold text-white leading-snug group-hover/card:text-primary transition-colors cursor-pointer"
                      ...
                      <div className="flex flex-wrap gap-2">
                        {post.tags.slice(0, 3).map(tag => (
                          <span key={tag} className="text-[10px] font-bold text-white/60 px-3 py-1.5 rounded-full bg-white/5 border border-white/5 flex items-center gap-1.5 hover:bg-white/10 hover:text-white transition-colors cursor-default">
                            <Hash className="w-2.5 h-2.5 text-primary/50" /> {tag}
=======
                    transition={{
                      type: "spring",
                      damping: 15,
...
<<<<<<< HEAD
                    <motion.button 
                      whileTap={{ scale: 0.95 }}
                      onClick={() => window.open(post.link, "_blank")}
                      className="mt-8 flex items-center justify-center gap-3 w-full py-3.5 rounded-2xl bg-white/5 hover:bg-primary text-white text-[11px] font-black transition-all border border-white/5 hover:border-primary hover:shadow-[0_0_20px_rgba(var(--primary-rgb),0.3)] relative z-10"
                    >
                      Analyze Context
=======
                    <motion.button
          ...
          <div className="mt-12 pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
            <div className="flex items-center gap-4">
              ...
              <p className="text-sm text-muted-foreground font-medium italic max-w-sm">
                "Our AI model synchronizes your progress with global developer trends to surface high-value connections."
              </p>
            </div>
            ...
            <div className="flex items-center gap-4 px-6 py-3 rounded-2xl bg-white/5 border border-white/5">
              <div className="flex -space-x-2">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="w-6 h-6 rounded-full border-2 border-background bg-gradient-to-br from-primary to-purple-500" />
                ))}
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-white uppercase tracking-tighter">1.2k Peers Online</span>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.6)]" />
                  <span className="text-[8px] font-bold text-muted-foreground uppercase">Sync Active</span>
=======
